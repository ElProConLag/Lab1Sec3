...
Para la primera actividad se desarrolló un programa en Python3 que implementa el algoritmo de cifrado César. El desarrollo se realizó tras un estudio detallado del algoritmo, sus fundamentos matemáticos y sus implicaciones en ciberseguridad. Se utilizaron herramientas de asistencia como referencia complementaria para validar la implementación.

\textbf{Especificaciones de implementación:}
\begin{quote}
Se implementó un programa en Python3 que maneja el cifrado César con desplazamiento configurable, preservando caracteres especiales y espacios. Se incluyó funcionalidad para descifrado y análisis automático basado en el estudio de técnicas criptoanalíticas clásicas.
\end{quote}
...
La segunda actividad consistió en desarrollar un programa que permite la transmisión encubierta de datos cifrados a través de paquetes ICMP, simulando tráfico normal de ping para evadir sistemas DPI. La implementación se basó en el análisis del protocolo ICMP y técnicas de esteganografía en redes.

\textbf{Enfoque de desarrollo:}
\begin{quote}
Se desarrolló un programa en Python3 que envía datos cifrados mediante paquetes ICMP Echo Request, colocando un carácter por paquete en el campo de datos. El diseño se fundamentó en el estudio de técnicas de evasión DPI y protocolos de red legítimos, asegurando que el tráfico sea indistinguible del ping normal.
\end{quote}
...
La tercera actividad desarrolló un programa capaz de interceptar y decodificar mensajes transmitidos mediante la técnica stealth, simulando un ataque MitM. La implementación se basó en principios de análisis de tráfico de red y técnicas criptoanalíticas.

\textbf{Metodología de implementación:}
\begin{quote}
Se implementó un programa en Python3 que captura paquetes ICMP y extrae datos ocultos aplicando técnicas de fuerza bruta sobre todos los desplazamientos César posibles (0-25). Se incorporó análisis heurístico de idioma para identificar el texto plano más probable, utilizando algoritmos de detección de patrones lingüísticos.
\end{quote}
...
\subsection*{Metodología de Aprendizaje y Desarrollo}

\subsubsection*{Proceso de desarrollo implementado:}
\begin{itemize}
    \item Estudio teórico de algoritmos criptográficos clásicos y sus vulnerabilidades
    \item Análisis de protocolos de red (ICMP) y técnicas de Deep Packet Inspection
    \item Implementación iterativa con validación de conceptos en cada fase
    \item Análisis crítico de las implicaciones de seguridad de cada técnica
\end{itemize}

\subsubsection*{Conceptos clave aplicados:}
\begin{itemize}
    \item Comprensión de la vulnerabilidad matemática del cifrado César
    \item Análisis de la estructura de paquetes ICMP para evasión DPI
    \item Aplicación de técnicas criptoanalíticas de fuerza bruta
    \item Evaluación crítica de métodos de detección lingüística
\end{itemize}

\subsubsection*{Validación y verificación:}
\begin{itemize}
    \item Pruebas exhaustivas de cada implementación contra casos de uso reales
    \item Validación de la efectividad de las técnicas de evasión DPI
    \item Análisis de las limitaciones y vulnerabilidades de cada enfoque
    \item Evaluación del impacto en seguridad desde perspectiva defensiva y ofensiva
\end{itemize}
...
Este laboratorio ilustra la realidad de que la seguridad por oscuridad no es suficiente. Los sistemas DPI deben evolucionar hacia análisis más sofisticados que consideren el comportamiento y contexto del tráfico, no solo patrones de contenido. La facilidad con que se implementaron estas técnicas resalta la importancia de comprender a fondo los protocolos de red y sus vulnerabilidades inherentes.

La accesibilidad de herramientas de desarrollo modernas democratiza tanto las capacidades ofensivas como defensivas en ciberseguridad, lo que hace más crucial que nunca la implementación de defensas multicapa y el monitoreo continuo de la red basado en análisis comportamental profundo.
